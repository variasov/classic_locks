...
from classic.locks import EXCLUSIVE, SHARED, MSAdvisoryLocker

from .common import *

DB_URL = 'mssql+pymssql://SA:123qweasdTRf#!@localhost:1433/test_db'
...
def locker_1(session):
    return MSAdvisoryLocker(session=session)
...
def excl_lock(session_2, resource_1_locker):
    return MSAdvisoryLocker(session=session_2).acquire(
